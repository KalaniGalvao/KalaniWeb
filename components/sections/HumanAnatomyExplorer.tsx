...
/**
 * Maps a pointer / tap to the NEAREST labelled structure, in the SVG's root
 * viewBox units. This replaces fragile per-path hit targets: small or
 * overlapping paths (the spine under the ribcage, abs under the waist, tiny
 * bones) used to swallow each other and were easy to miss on touch. Picking the
 * closest marker guarantees every region — including the central thorax, skull
 * and beyond — is reachable on mouse AND touch, using only getScreenCTM +
 * DOMPoint (supported in every modern browser, including Safari). Returns null
 * when the pointer is well away from the figure.
 */
function pickPartId(
  e: { currentTarget: EventTarget | null; clientX: number; clientY: number },
  layer: AnatomyLayer,
): string | null {
  ...
  const { s, tx, ty } = layer.align;
  let bestId: string | null = null;
  let bestDist = Infinity;
  for (const part of layer.parts) {
    const mx = part.marker.x * s + tx;
    const my = part.marker.y * s + ty;
    const dist = (local.x - mx) ** 2 + (local.y - my) ** 2;
    if (dist < bestDist) {
      bestDist = dist;
      bestId = part.id;
    }
  }
  // ~250 user-unit reach: anywhere on the figure resolves to a structure, but
  // taps out in the empty park clear the selection.
  return bestDist <= 250 * 250 ? bestId : null;
}

export function HumanAnatomyExplorer({
  segment,
  ctaHref = "#contato",
  autoPlay = false,
}: {
  ...
  ctaHref?: string;
  /** Dedicated-page intro: reveal copy + auto-cycle the layers without scrolling. */
  autoPlay?: boolean;
}) {
  ...
  // Mobile-only: collapse the copy so the model fills the screen for analysis.
  ...
  // Auto-play intro (dedicated page) stops cycling once the visitor engages.
  ...
  // First-visit / nav auto-play of the copy fade-in on the main page; hands off
  // to scroll afterwards. Disabled on the dedicated page (autoPlay) and reduced
  // motion. Layer switching stays scroll-driven.
  const introProgress = useIntroProgress({
    id: segment.id,
    sectionRef,
    scrollYProgress,
    durationMs: 1600,
    // Main page only. The dedicated page fades the copy in via a self-contained
    // mount animation (below) so it can never be blanked by the per-load "played"
    // flag when the same section id was already seen on the home page.
  ...
  const healthTextY = useTransform(introProgress, [0.18, 0.34], [34, 0]);
  ...
  // Auto-play intro (dedicated page): cycle body → skeleton → organs so the
  // capability is visible without scrolling; stops once the visitor engages.
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      const next = (layerIndexRef.current + 1) % LAYERS.length;
    ...
    <section
      ...
      className={
        autoPlay
          ...
          : "relative min-h-[250vh] md:min-h-[300vh]"
      }
    >
      <div className="sticky top-0 h-[100svh] overflow-hidden bg-[#07101f]">
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Region outlines are visual-only (pointer-events:none via the
                    .anatomy-zone CSS). All hit-testing is done by the single
                    transparent rect below, which maps the pointer/tap to the
                    NEAREST labelled structure — so the central thorax, skull and
                    overlapping bones are all reachable on mouse and touch (incl.
                    Safari). The CSS :hover/touch rules reveal the outlines. */}
                <g className="anatomy-zone">
                  ...
                  {/* Single hit target over the figure (root viewBox coords).
                      onClick covers tap + click everywhere (incl. Safari); the
                      mouse-only pointer handlers add hover highlighting and clear
                      on leave (touch selections persist until the next tap). */}
                  {isActive && (
                    <rect
                      x={1020}
                      y={150}
                      ...
                      onClick={(event) => {
                        autoCycleStoppedRef.current = true;
                        setActivePartId(pickPartId(event, item));
                      }}
                      onPointerMove={(event) => {
                        if (event.pointerType !== "mouse") return;
                        autoCycleStoppedRef.current = true;
                        const id = pickPartId(event, item);
                        setActivePartId((prev) => (prev === id ? prev : id));
                      }}
                      onPointerLeave={(event) => {
                        if (event.pointerType === "mouse") setActivePartId(null);
                      }}
                    />
                  )}
                </g>
        ...
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-0 top-0 z-20 h-16 bg-gradient-to-b from-[#02030a] via-[#02030acc] to-transparent sm:h-48"
        />

        {/* Readability scrim. On mobile it only darkens the very top (tabs/copy)
            and stays CLEAR over the model so the head and torso read well;
            desktop keeps the left-side scrim for the left-aligned copy. */}
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,rgba(4,9,20,.72)_0%,rgba(4,9,20,.16)_20%,transparent_46%,transparent_100%)] md:bg-[linear-gradient(90deg,rgba(4,9,20,.96)_0%,rgba(4,9,20,.88)_26%,rgba(4,9,20,.48)_48%,rgba(4,9,20,.08)_72%,rgba(4,9,20,.12)_100%)]"
        />

        {/* Bottom edge-blend — melts the bright park floor into the dark so the
            handoff to the beauty section below is a seamless dark valley (the
            beauty scene fades UP into this same near-black, no hard line). */}
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-0 bottom-0 z-20 h-28 bg-gradient-to-t from-[#02030a] via-[#02030acc] to-transparent sm:h-40"
        />
        ...
        <div className="container-rail pointer-events-none relative z-40 flex h-full items-start pt-[calc(env(safe-area-inset-top)+5.25rem)] md:items-center md:pt-0">
          <motion.article
            // Dedicated page: fade the copy in on every mount (never gated by a
            // per-load flag). Main page: driven by the intro/scroll progress.
            initial={autoPlay && !reduce ? { opacity: 0, y: 24 } : false}
            animate={autoPlay && !reduce ? { opacity: 1, y: 0 } : false}
          ...
          >
            <div className={infoHidden ? "hidden md:block" : ""}>
              <span className="inline-flex items-center gap-2 rounded-full border border-[#54e6b5]/40 bg-[#54e6b5]/10 px-3.5 py-1.5 font-mono text-[0.6rem] uppercase tracking-[0.22em] text-[#79f7cb] backdrop-blur-md sm:text-xs">
                <svg
                  aria-hidden="true"
                  viewBox="0 0 24 24"
            ...
            <div className={infoHidden ? "hidden md:block" : ""}>
              ...
              <div className="pointer-events-auto mt-3 flex flex-wrap gap-2.5 md:mt-8">
                <a
                  href={WHATSAPP_URL}
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-full bg-[#54e6b5] px-4 py-2 text-sm font-medium text-[#04140f] shadow-lg transition hover:brightness-110 md:px-6 md:py-3 md:text-base"
                >
                  Começar projeto
                </a>
                <button
                  type="button"
                  onClick={goToCta}
                  className="rounded-full border border-[#54e6b5]/45 bg-black/10 px-4 py-2 text-sm font-medium text-white backdrop-blur-md transition hover:border-[#54e6b5]/80 hover:bg-[#54e6b5]/10 md:px-6 md:py-3 md:text-base"
                >
                  Saiba mais
                </button>
              </div>

              {/* Trust signals — desktop only so the mobile hero stays compact. */}
              <div className="mt-6 hidden flex-wrap items-center gap-x-6 gap-y-2 text-xs text-white/70 md:mt-9 md:flex">
                <span className="inline-flex items-center gap-2">
                  <svg
                    aria-hidden="true"
                    viewBox="0 0 24 24"
                    className="h-4 w-4 text-[#54e6b5]"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth={2}
                    strokeLinecap="round"
        ...
        {/* Mobile: Hide/Show info pinned to the BOTTOM so it never covers the
            model's head — tap to analyse the model full-screen, then restore. */}
        <button
          type="button"
          onClick={() => setInfoHidden((value) => !value)}
          aria-pressed={infoHidden}
          className="pointer-events-auto absolute bottom-[calc(env(safe-area-inset-bottom)+1.25rem)] left-1/2 z-40 inline-flex -translate-x-1/2 items-center gap-2 rounded-full border border-[#54e6b5]/45 bg-black/55 px-5 py-2.5 text-xs font-medium text-white shadow-lg backdrop-blur-md transition hover:border-[#54e6b5]/80 md:hidden"
        >
          <span
            aria-hidden="true"
            className="h-1.5 w-1.5 rounded-full bg-[#54e6b5]"
          />
          {infoHidden ? "Mostrar informações" : "Ocultar informações"}
        </button>
      </div>
    </section>
  );
