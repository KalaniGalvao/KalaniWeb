    ...
    >
      <div className="sticky top-0 h-[100svh] overflow-hidden bg-[#07101f]">
        ...
        <div className="container-rail pointer-events-none relative z-40 flex h-full items-start pt-[calc(env(safe-area-inset-top)+5.25rem)] md:items-center md:pt-0">
          ...
          >
            {/* Mobile-only: collapse the copy so the model can be analysed
                full-screen, then expand it again. Hidden on md+ (room there). */}
            <button
              type="button"
              onClick={() => setInfoHidden((value) => !value)}
              aria-pressed={infoHidden}
              className="pointer-events-auto mb-3 inline-flex items-center gap-2 rounded-full border border-[#54e6b5]/45 bg-black/40 px-4 py-1.5 text-xs font-medium text-white backdrop-blur-md transition hover:border-[#54e6b5]/80 md:hidden"
            >
              <span
                aria-hidden="true"
