...
export function HumanAnatomyExplorer({ segment }: { segment: Segment }) {
  ...
  return (
    <section
      ...
      className="relative min-h-[100svh] md:min-h-[300vh]"
    >
      <div className="relative h-[100svh] overflow-hidden bg-[#07101f] md:sticky md:top-0">
            ...
            >
              ...
              >
                ...
                <g
                  ...
                  onPointerLeave={() => {
                    if (!isMobileViewport) setActivePartId(null);
                  }}
                >
                  <ellipse
                    cx={FIGURE.cx}
                  ...
                  <g transform={figureTransform}>
                    ...
                    {isMobileViewport && isActive && (
                      <g aria-hidden="true" className="md:hidden">
                        {item.parts.map((part) => {
                          const isHeadLike =
                            part.id.includes("head") ||
                            part.id.includes("skull") ||
                            part.id.includes("mandible");
                          const isHandOrFoot =
                            part.id.includes("hand") ||
                            part.id.includes("foot");
                          const isCore =
                            part.id.includes("chest") ||
                            part.id.includes("ribcage") ||
                            part.id.includes("abs") ||
                            part.id.includes("waist") ||
                            part.id.includes("pelvis") ||
                            part.id.includes("intestine") ||
                            part.id.includes("stomach");

                          const rx = isCore ? 56 : isHeadLike ? 44 : 38;
                          const ry = isCore ? 48 : isHandOrFoot ? 34 : 40;

                          return (
                            <ellipse
                              key={`${part.id}-mobile-tap-zone`}
                              cx={part.marker.x}
                              cy={part.marker.y}
                              rx={rx}
                              ry={ry}
        ...
        <motion.div
          aria-hidden="true"
          style={{
            opacity: isMobileViewport || reduce ? 0 : lawyersToHealthBlend,
          }}
        ...
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-0 top-0 z-20 hidden h-48 bg-gradient-to-b from-[#02030a] via-[#02030acc] to-transparent md:block"
        />

        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 hidden md:block md:bg-[linear-gradient(90deg,rgba(4,9,20,.96)_0%,rgba(4,9,20,.88)_26%,rgba(4,9,20,.48)_48%,rgba(4,9,20,.08)_72%,rgba(4,9,20,.12)_100%)]"
        />

        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-0 bottom-0 z-20 h-44 bg-gradient-to-b from-transparent via-[#07101f]/70 to-[#f1ebe3] md:hidden"
        />
        ...
        <div className="container-rail pointer-events-none relative z-40 flex h-full items-start pt-[calc(env(safe-area-inset-top)+5.25rem)] md:items-center md:pt-0">
          ...
          <motion.article
            style={{
              opacity: reduce || isMobileViewport ? 1 : healthTextOpacity,
              y: reduce || isMobileViewport ? 0 : healthTextY,
            }}
            className="pointer-events-none max-w-[min(92vw,21rem)] rounded-[1.5rem] bg-[#02030a]/35 p-4 backdrop-blur-md md:pointer-events-auto md:max-w-xl md:rounded-none md:bg-transparent md:p-0 md:py-20 md:backdrop-blur-none"
          >
            <p
        ...
        {isMobileViewport && mobileCopyHidden && (
          ...
          {activePart && (
            <motion.div
              key={`${layer.id}-${activePart.id}-mobile-readout`}
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.18 }}
              className="pointer-events-none absolute inset-x-4 bottom-[calc(env(safe-area-inset-bottom)+5.7rem)] z-50 rounded-2xl border border-[#65efc1]/25 bg-[#02030a]/62 p-3 text-white shadow-2xl backdrop-blur-xl"
              aria-live="polite"
            >
              <p className="font-mono text-[0.62rem] uppercase tracking-[0.22em] text-[#79f7cb]/85">
                {layer.tab}
              </p>
